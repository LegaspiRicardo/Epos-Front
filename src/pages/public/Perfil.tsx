...
const Perfil: React.FC = () => {
  const [seccionActiva, setSeccionActiva] = useState<
    "pedidos" | "domicilios" | "cotizaciones" | "favoritos"
  >("domicilios");
  const [selectedCotizacion, setSelectedCotizacion] =
    useState<Cotizacion | null>(null);
  const [selectedPedido, setSelectedPedido] = useState<PedidoPerfil | null>(
    null
  );
  const [selectedDomicilio, setSelectedDomicilio] =
    useState<DomicilioPerfil | null>(null);

  const [showModalDomicilio, setShowModalDomicilio] = useState(false);
  const [modalDomicilioMode, setModalDomicilioMode] = useState<
    "view" | "create"
  >("view");
      ...
      <section>
        <div className="sm:w-11/12 mx-auto w-full py-4">
          <div className="flex md:w-8/12 xl:w-4/12 w-11/12 mx-auto ">
            {/* IMAGEN */}
            <div className=" w-2/6">
              <p className="bg-blue-200 text-8xl text-center pb-3 md:px-3 md:w-8/12 mx-auto rounded-full">
                ○
              </p>
            </div>
            {/* INFORMACIÓN CUENTA */}
            <div className="w-4/6 ml-auto pl-4 mt-2 md:mt-0 ">
              <div className="">
                <div className="flex">
                  ...
                  <button className=" mx-auto rounded-xl  mt-2 p-2">
                    <img
                      src="/icons/lapiz.png"
                      alt="Icono editar"
                      className="w-6 h-6"
          ...
          <div className="w-11/12 mx-auto md:w-8/12 xl:w-4/12">
            <button className="mt-6 w-full mx-auto border-red-600 border text-red-600 py-1 rounded-xl">
              Cerrar sesión
            </button>
          </div>
        </div>
          ...
          <button
            className={`w-2/6 py-2 ${
              seccionActiva === "pedidos"
                ? "bg-cyan-800 text-white rounded-lg"
                : ""
            }`}
            ...
            {pedidosData.map((pedido) => (
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-medium ${getEstadoPedidoColor(
                      pedido.estado
                    )}`}
                  >
                ...
                <div className="flex justify-between items-center text-sm text-gray-600 mb-3">
                  <div>
                    ...
                    <p>
                      <strong>Productos:</strong> {pedido.items.length} item(s)
                    </p>
                    {pedido.trackingNumber && (
                      <p>
                        <strong>Tracking:</strong> {pedido.trackingNumber}
                      </p>
                    )}
                  </div>

                  <p className="text-lg font-bold text-cyan-800">
                    ${pedido.total.toLocaleString()} MXN
                  </p>
                </div>
              ...
              >
                ...
                <div className="flex justify-between items-center text-sm text-gray-600 mb-3">
                  <div>
                    ...
                    <p>
                      <strong>Productos:</strong> {cotizacion.items.length}{" "}
                      item(s)
                    </p>
                  </div>
                  ...
                  <div className="my-4 w-full bg-cyan-900 text-white shadow-lg py-5 px-2 transition-transform hover:scale-105 duration-300">
                    <div className="flex w-full">
                      ...
                      <div className="w-7/12 ml-4">
                        <div className="flex">
                          <h3 className="text-xl font-semibold mb-1 w-4/6 text-justify">
                            {card.nombre}
                          </h3>
                          <p className="mx-auto rounded-full px-2 pt-1 bg-slate-500">
                            ⭐
                          </p>
                        </div>
            ...
            {domiciliosData.length > 0 ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-400 text-sm mb-4">
                  Agrega tu primer domicilio para comenzar
                </p>
                <button
                  onClick={handleCreateDomicilio}
